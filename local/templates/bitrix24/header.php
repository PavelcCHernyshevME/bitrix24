...
<!--						<span class="header-logo-block-util"></span>-->
						...
						{
							$showCake = ($licensePrefix == "ua" && $now > gmmktime(0, 0, 0, 10, 2, 2018) && $now < gmmktime(20, 59, 59, 10, 2, 2018));
							?>
							<a id="logo_24_a" href="<?=htmlspecialcharsbx(SITE_DIR)?>" title="<?=GetMessage("BITRIX24_LOGO_TOOLTIP")?>" class="logo"><?
								...
								?>
								<span class="logo-text-container" id="logo_24_text" <?if ($clientLogo || $clientLogoRetina):?>style="display:none"<?endif?>>
									<span class="logo-text"><?=htmlspecialcharsbx($siteTitle)?></span>
									<?if(COption::GetOptionString("bitrix24", "logo24show", "Y") !=="N" && !$showCake):?>
										<span class="logo-color">24</span>
									<?endif?>
								</span>
								<span class="logo-img-span">
								...
								<?if ($showCake):?>
									<span style="
										...
										height: 39px;
										margin-left: 7px;
										background: no-repeat center url('/local/templates/bitrix24images/cake.png');
										background-size: cover;
									" onclick="window.open('https://www.bitrix24.ua/~5Y');"></span>
								<?endif?>
							</a>
							<?
